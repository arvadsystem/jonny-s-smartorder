...
const Inventario = () => {
  ...
  const validarCategoria = (data) => {
    // NORMALIZAR (TRIM + MAYÚSCULAS EN CÓDIGO)
  ...
  const guardarEdicion = async () => {
    ...
    try {
      // TU SP ACTUALIZA 1 CAMPO A LA VEZ
      const updates = [
  ...
  // ==============================
  // ORDENAR + FILTRAR (NO DEPENDER DEL ID)
  // ==============================
  const categoriasFiltradas = useMemo(() => {
    // ORDENAMOS POR ID PARA QUE EL LISTADO SEA ESTABLE
  ...
  return (
    <div className="container-fluid p-3">
      ...
      <div className="card shadow-sm mb-3">
        <div className="card-header fw-semibold">Categorías de productos</div>

        <div className="card-body">
          {/* ALERTA GENERAL (LA DEJAMOS POR SIEMPRE) */}
          {error && <div className="alert alert-danger">{error}</div>}

          {/* FORM CREAR (RESPONSIVE) */}
                        ...
                        <div className="d-flex justify-content-between align-items-start mb-2">
                          <div>
                            ...
                            <div className="fw-bold">
                              {isEditing ? 'EDITANDO' : c.nombre_categoria}
                            </div>
                            <div className="text-muted small">
                              Código: <span className="fw-semibold">{c.codigo_categoria}</span> • Estado:{' '}
                              {isEditing ? (editForm?.estado ? 'Activo' : 'Inactivo') : (c.estado ? 'Activo' : 'Inactivo')}
                            </div>
                          </div>
                        </div>
          ...
          {/* ==============================
              VISTA ESCRITORIO (TABLA) -> SOLO >= md
              ============================== */}
      ...
      {/* ==============================
          MODAL CONFIRMACIÓN ELIMINAR (PRO)
          ============================== */}
      {confirmModal.show && (
        <div
