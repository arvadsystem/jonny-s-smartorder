...
export default function SolicitudesCompraTab({ openToast }) {
  ...
  if (flow.view === 'detalle') return <SolicitudCompraDetalle state={flow.detailState} onBack={() => flow.setView('listado')} onRetry={() => flow.openDetail(flow.detailState.id)} reloadDetail={() => flow.openDetail(flow.detailState.id)} reloadList={() => flow.loadList({ page: Number(flow.listState.pagination?.page || 1), estado: flow.filter })} canApprove={canApprove} canReject={canReject} openToast={openToast} />;
  return <SolicitudesCompraListado state={flow.listState} filter={flow.filter} onFilter={flow.setFilter} onPage={(page) => flow.loadList({ page, estado: flow.filter })} onDetail={flow.openDetail} onCreate={flow.openCreate} canCreate={canCreate} canReview={canApprove || canReject} />;
}
