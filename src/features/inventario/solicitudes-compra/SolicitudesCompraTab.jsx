...
export default function SolicitudesCompraTab({ openToast }) {
  ...
  return <SolicitudesCompraListado state={flow.listState} filter={flow.filter} onFilter={flow.setFilter} search={flow.search} onSearch={flow.setSearch} onClearSearch={flow.clearSearch} onPage={(page) => flow.loadList({ page, estado: flow.filter, buscar: flow.search })} onDetail={flow.openDetail} onCreate={flow.openCreate} onRefresh={() => flow.loadList({ page: Number(flow.listState.pagination?.page || 1), estado: flow.filter, buscar: flow.search })} openToast={openToast} canCreate={canCreate} canReview={canApprove || canReject} canReject={canReject} canReceive={canReceive} />;
}
