...
import SolicitudCompraRechazoRapido from './SolicitudCompraRechazoRapido';
...
export default function SolicitudesCompraListado({ state, filter, onFilter, search, onSearch, onClearSearch, onPage, onDetail, onCreate, onQuickCapture, onQuickCaptureAdmin, onRefresh, openToast, canCreate, canQuickCaptureCreate, canQuickCaptureAdmin, canReview, canReject, canReceive }) {
  ...
  return (
    <section className="sol-comp-section" aria-labelledby="sol-comp-title">
      <header className="sol-comp-header">
        ...
        <div className="sol-comp-header-actions">{canCreate ? <button type="button" className="btn btn-primary sol-comp-primary-action sol-comp-action-with-help" onClick={onCreate}><i className="bi bi-plus-circle" aria-hidden="true" /><span>Nueva solicitud<small>Solicitud completa</small></span></button> : null}{canQuickCaptureCreate ? <button type="button" className="btn btn-outline-primary sol-comp-action-with-help" onClick={onQuickCapture}><i className="bi bi-camera" aria-hidden="true" /><span>Compra rápida<small>Solo factura</small></span></button> : null}{canQuickCaptureAdmin ? <button type="button" className="btn btn-outline-secondary sol-comp-action-with-help" onClick={onQuickCaptureAdmin}><i className="bi bi-inboxes" aria-hidden="true" /><span>Capturas rápidas<small>Facturas por formalizar</small></span></button> : null}</div>
      </header>
