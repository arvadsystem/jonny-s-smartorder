...
function CatalogItem({ item, onAdd }) {
  ...
  const add = () => {
    ...
    onAdd({
      ...
      equivalencia: selected ? `${parsed} ${selected.nombre_presentacion} ≈ ${parsed * Number(selected.factor_conversion || selected.cantidad_base || 1)} ${selected.unidad_base || item.unidad_base}` : null
    });
    setQuantity('');
  };
...
export default function SolicitudCompraCatalogo({ warehouseId, state, loadCatalog, onAdd }) {
  ...
  return (
    <section className="sol-comp-catalog" aria-labelledby="catalog-title">
      ...
      <div className="sol-comp-catalog-grid">{state.items.map((item) => <CatalogItem key={`${item.tipo_item}-${item.id_item}`} item={item} onAdd={onAdd} />)}</div>
      {!state.loading && !state.error && !state.items.length ? <div className="sol-comp-empty">No hay artículos para estos filtros.</div> : null}
