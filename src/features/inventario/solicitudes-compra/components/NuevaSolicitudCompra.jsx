...
export default function NuevaSolicitudCompra({ warehouses, warehousesLoading, catalogState, loadCatalog, submit, createPhase = 'IDLE', invalidatePending, retryPending, onBack, openToast }) {
  ...
  const locked = submitting || createPhase === 'SENDING' || createPhase === 'RECONCILING';
  ...
  const send = async () => {
    if (locked || !warehouseId || !allValid) return;
    setSubmitting(true);
    try {
  ...
  return (
    <section className="sol-comp-section" aria-busy={locked}>
      <button type="button" className="sol-comp-back" disabled={locked} onClick={onBack}><i className="bi bi-arrow-left" aria-hidden="true" /> Volver a solicitudes</button>
      ...
      {locked ? <div className="sol-comp-feedback" role="status" aria-live="assertive"><span className="spinner-border spinner-border-sm" /> {createPhase === 'RECONCILING' ? 'Verificando envío…' : 'Enviando solicitud…'}</div> : null}
      {createPhase === 'AWAITING_RETRY' ? <div className="sol-comp-feedback sol-comp-feedback--error" role="status"><strong>NO PUDIMOS CONFIRMAR TODAVÍA</strong><br />No vuelvas a crear la solicitud. Puedes reintentar el mismo envío de forma segura. <button type="button" className="btn btn-warning btn-sm" onClick={() => retryPending?.()}>Reintentar envío</button></div> : null}
      ...
      <div className="sol-comp-warehouse">
        ...
        <div className="sol-comp-warehouse__select">
          <AppSelect label="Almacén" placeholder={warehousesLoading ? 'Cargando almacenes…' : 'Selecciona un almacén'} value={warehouseId} options={warehouseOptions} onChange={(selected) => { invalidatePending?.(); setWarehouseId(selected); setLines([]); }} disabled={warehousesLoading || locked} searchable={warehouseOptions.length > 6} />
        </div>
      </div>
      {!warehouseId ? <div className="sol-comp-feedback">Selecciona un almacén para consultar su catálogo.</div> : (
        <div className="sol-comp-create-layout">
          ...
          <SolicitudCompraResumen lines={lines} onChange={(index, cantidad) => { invalidatePending?.(); setLines((current) => current.map((line, currentIndex) => currentIndex === index ? { ...line, cantidad } : line)); }} onRemove={(index) => { invalidatePending?.(); setLines((current) => current.filter((_, currentIndex) => currentIndex !== index)); }} observation={observation} setObservation={(value) => { invalidatePending?.(); setObservation(value); }} submitting={locked} onSubmit={send} disabled={!warehouseId || !allValid} />
        </div>
      )}
      </fieldset>
    </section>
