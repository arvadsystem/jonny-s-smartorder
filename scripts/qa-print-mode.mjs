...
const printActions = fs.readFileSync(new URL('../src/pages/dashboard/ventas/utils/ventasPrintActions.js', import.meta.url), 'utf8');
const ventasService = fs.readFileSync(new URL('../src/services/ventasService.js', import.meta.url), 'utf8');
const kitchenRouting = fs.readFileSync(new URL('../src/pages/dashboard/ventas/utils/ventasKitchenRouting.js', import.meta.url), 'utf8');
...
assert.match(printActions, /sourceType:\s*'pedido'[\s\S]*action:\s*'initial'[\s\S]*origin:\s*'pending-order'/);
const pendingPromptStart = page.indexOf('const handlePendingOrderCreatedPrintPrompt');
const pendingPromptEnd = page.indexOf('const handleSuccessfulPendingOrderPaymentPrint', pendingPromptStart);
assert.ok(pendingPromptStart >= 0 && pendingPromptEnd > pendingPromptStart);
const pendingPromptFlow = page.slice(pendingPromptStart, pendingPromptEnd);
assert.match(pendingPromptFlow, /resolvePendingOrderComandaPrompt/);
assert.match(pendingPromptFlow, /setComandaPrompt\(resolution\.prompt\)/);
assert.doesNotMatch(pendingPromptFlow, /AGENT_PRINT_MODE|executeComandaPrint|enqueueAgentPrintAction/);
...
assert.match(page, /loadKitchenComandaPrintSource/);
assert.match(kitchenRouting, /sourceType === 'pedido'[\s\S]*ventasApi\.getPedidoComanda\(venta\?\.id_pedido\)/);
